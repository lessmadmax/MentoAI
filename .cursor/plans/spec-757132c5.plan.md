<!-- 757132c5-bfdd-491f-a7ef-fd850dd5c7ad 1d436e7a-ac3b-4800-b884-7cda7621cbaa -->
...
1. **1단계 (긴급)**: 500 에러 해결
   ...
   - Repository 삭제 메서드 생성
   - UserProfileService 재작성
   - 테스트 및 검증
   - 실패 시 전략 B 시도
...
- [ ] Fix ObjectOptimisticLockingFailureException in UserProfileService - remove refresh(), improve transaction handling, add retry logic
- [ ] Verify user profile data persistence - test all fields, collections, and child entities are saved correctly
- [ ] Set up Gemini API configuration - add properties, create GeminiConfig class, configure HTTP client
- [ ] Implement GeminiService - text generation, embeddings, profile analysis methods
- [ ] Integrate Gemini API with existing services - profile enhancement, semantic search improvements
- [ ] Design ETL pipeline integration architecture - define data sync patterns, webhook structure
- [ ] Implement ETLIntegrationService - sync methods, webhook receiver, error handling
- [ ] Create ETLController with sync, webhook, and status endpoints
- [ ] Configure scheduled tasks for periodic ETL data synchronization